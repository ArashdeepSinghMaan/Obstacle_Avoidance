...
//Leader Follower ,Follower with DMM
...
public:
          ...
          pid_wamv_yaw_(1.0, 0.2, 0.3) // PID for WAM-V yaw control
    {
        ...
        desired_wamv_x_ = 15.0;
        desired_wamv_y_ = 15.0;
...
double safe_distance_ = 5.0;  // Example safe distance
...
//;;
    ...
    void wamv_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
       ...
       RCLCPP_INFO(this->get_logger(),"desired wamv yaw ,[%.2f]",desired_wamv_yaw_);
    ...
    double thrust_yaw_2{0};
      ...
      if(yaw_error>0){
         if (yaw_error<0.70){
        thrust_yaw_1 =1.5*yaw_error;
         thrust_yaw_2= -1.5*yaw_error;}
         else if (yaw_error>0.70){
        thrust_yaw_1 =4*yaw_error;
         thrust_yaw_2= -4*yaw_error;}
      }
      else if(yaw_error<0){
         if (yaw_error > -0.70){
        thrust_yaw_1 =-1.5*yaw_error;
       ...
       thrust_yaw_2=std::clamp(thrust_yaw_2, -5.0, 5.0);
        ...
        if( error_x<=1.5 && error_x>=-1.5){
            left_thrust=0;
            right_thrust=0;
        }
        thrust_x = std::clamp(thrust_x, -10.0, 10.0);
        right_thrust=std::clamp(right_thrust, -15.0, 15.0);
        left_thrust=std::clamp(left_thrust, -15.0, 15.0);
        ...
        RCLCPP_INFO(this->get_logger(), "WAMV Thrusters: Left = %.2f, Right = %.2f,left_angle:[%.2f],right_angle:[%.2f]", left_thrust, right_thrust, thrust_yaw_2, thrust_yaw_1);
    }
